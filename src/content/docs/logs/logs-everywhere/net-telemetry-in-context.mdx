---
title: '.NET telemetry in context'
...
[Telemetry in context](https://docswebsitedevelop-doc7761logsintro.gtsb.io/docs/logs/logs-everywhere/telemetry-in-context/) for the .NET agent connects your logs with all of your telemetry data for your apps, hosts, and other entities. Bringing all of this data together in a single tool helps you quickly:
...
![Telemetry in context in APM UI](./images/net-errors-log-details.png "Telemetry in context in APM UI")
...
The following .NET logging frameworks are supported:

| Framework | Supported Versions                           |
| --------- | -------------------------------------------- |
| Serilog   | >= 2.0.0                                     |
| log4net   | .NET Framework: >= 1.2.10                    |
|           | .NET Core: >= 2.0.6                          |
| Microsoft.Extensions.Logging | >= 2.0.0 (.NET Core only) |
...
<CollapserGroup>
  ...
  All you need to do is install an agent version with log forwarding capabilities ([.NET agent 9.7.0 or higher](https://docs.newrelic.com/docs/release-notes/agent-release-notes/net-release-notes/)), and update your configuration to enable forwarding.
  ...
  **Optional adjustments:**
  ...
  1. Modify your application's logging configuration to include the `NR_LINKING` property:

  `Serilog` example:
  ```
  loggerConfig
    .MinimumLevel.Debug()
    .WriteTo.File(path: "C:\path\to\your\logs\file.log", 
  ...
  fileAppender.File = "C:\path\to\your\logs\file.log";
  fileAppender.AddFilter(filter);

  PatternLayout patternLayout = new PatternLayout();
  patternLayout.ConversionPattern = "%timestamp [%thread] %level %logger - %message %property{NR_LINKING} %newline";
  ...
  `Microsoft.Extensions.Logging` example (using Serilog as the third-party logging provider):
  ```
  ...
  using var loggerFactory = LoggerFactory.Create(builder =>
  {
    builder
        ...
        .AddFilter("MyApplication", LogLevel.Debug)
        .AddSerilog();
  });
  
  logger = loggerFactory.CreateLogger<MyApplication>();
  ```
  
  For `Microsoft.Extensions.Logging`, note that the linking metadata is added to the `{Scope}` property rather than the `{NR_LINKING}` property.
  ...
  >
  Before language agents had the ability to forward and decorate logs, you could enable logs in context by updating your application to use a [Logging enricher](https://github.com/newrelic/newrelic-logenricher-dotnet) to send linking metadata.

  This option is still supported, but is no longer encouraged. For instructions to use this approach, see [.NET: Configure logs in context](https://docs.newrelic.com/docs/logs/logs-context/net-configure-logs-context-all/).
