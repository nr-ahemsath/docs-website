...
4. Restart affected applications. 
    ...
    ```powershell
    msiexec.exe /i C:\PATH_TO\newrelic-agent-win-x86-VERSION.msi /qb NR_LICENSE_KEY=YOUR_LICENSE_KEY NR_APP_NAME=YOUR_APP_NAME INSTALLLEVEL=1
    ```
    ...
    * `50`: Installs the agent with InstrumentAllNETFramework enabled (described below).
    ...
    ```powershell
    msiexec.exe /i C:\PATH_TO\newrelic-agent-win-x86-VERSION.msi /qb NR_LICENSE_KEY=YOUR_LICENSE_KEY NR_APP_NAME=YOUR_APP_NAME ADDLOCAL=OPTION_1, OPTION_2
    ```
